...
const Login = () => {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      alert("อีเมลหรือรหัสผ่านผิดพลาด!");
    }
  };

  return (
    <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', height: '100vh', background: '#f8fafc' }}>
      <form onSubmit={handleLogin} style={{ background: 'white', padding: '30px', borderRadius: '15px', boxShadow: '0 10px 25px rgba(0,0,0,0.1)', width: '320px' }}>
        <h2 style={{ textAlign: 'center', color: '#1e293b' }}>🔒 Admin Access</h2>
        ...
        <button type="submit" style={{ width: '100%', padding: '12px', background: '#1e293b', color: 'white', border: 'none', borderRadius: '8px', fontWeight: 'bold', cursor: 'pointer' }}>ล็อกอิน</button>
      </form>
    </div>
  );
};
